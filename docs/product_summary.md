...
*   **Security Teams (CISOs):** TSP's spectral engine acts as an "Executive Function." By monitoring the algebraic connectivity ($\lambda_2$) of the graph, it provides a mathematical foundation for detecting abrupt semantic shifts and identifying topological anomalies associated with prompt injection attacks.
